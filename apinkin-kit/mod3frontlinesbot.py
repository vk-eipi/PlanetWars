# all in radius in frontlines; rally targets weaker frontiers; frontlines send self "bug fixed
# wins 39:10(1)[Lt111/Wt121] when player 1 against rallybot

'''
Uses alternative python starter kit by RebelXT
'''
from planetwars import BaseBot, Game
from planetwars.universe_custom import MyUniverse3
...
class MyBot(BaseBot):
#key changes (score -> frontlines):
#   ship availables to front lines
    ...
    def do_turn(self):
        ...
        if len(my_planets) == 1 and len(self.universe.enemy_planets) == 1 and len(self.universe.all_fleets) == 0:
            ...
            #rally to frontier planets
            if len(self.universe.frontlines) > 0:
                log.debug("front lines: %s" % self.universe.frontlines)
                for p in my_planets:
                    if (p not in self.universe.frontlines) and (p.available*MyBot.fraction_available >= 1):
                        toAttack = int(p.available*MyBot.fraction_available)
                        supply_target = min(self.universe.frontlines, key = lambda x: (p.distance(x), x.ship_count, x.id))
                        p.send_fleet(supply_target, toAttack)
                        log.debug("Rallying to %s from %s; %d out of %d available." % (supply_target, p, toAttack, p.available))
                        p.available -= toAttack

Game(MyBot, universe_class=MyUniverse3, planet_class=Planet2)
